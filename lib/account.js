...
const { REDIS_PREFIX, ACCOUNT_DELETED, MAILBOX_HASH } = require('./consts');
...
class Account {
    ...
    async getActiveAccessTokenData() {
        ...
        if (accountData.oauth2.useAuthServer) {
            // resolve credentials
            let authData = this.tokenCache.get(this.account);
            const expiresAt = authData.expires || 0;
            if (!authData || Date.now() > expiresAt) {
                authData = await resolveCredentials(this.account, 'api');
                this.tokenCache.set(this.account, authData);
            }
    ...
    async oauth2Request(url, method, payload, options) {
        ...
        if (oAuth2Client.apiBase) {
            let urlObj = new URL(oAuth2Client.apiBase, url);
            url = urlObj.href;
        }
